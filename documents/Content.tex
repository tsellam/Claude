...
\begin{algorithmic}
    ...
    \EndFor
    \State \Return $\text{findTopK}(\text{Beam}, \text{Scores}, K)$
    \EndFunction
\end{algorithmic}
\end{algorithm}
...
must generate $B' > B$ views beforehand. A low $B'$ will lead to more variety,
while a high $B'$ may lead to better views.
...
for a simple approach. First, we compute the dissimilarity between every pair
of views. Then, we then run a clustering algorithm over the resulting
dissimilarity matrix. To quantify the dissimilarity between the views, we use
the Jaccard distance. Given two views $V_i$ and $V_j$, this distance defined as
follows: $d_J(V_i, V_j) = |V_i \cap V_j | / | V_i \cup V_j |$. For the
clustering, we use Partitioning Around Medoids, an algorithm of type k-medoids.
We refer the interested reader to the literature for more
details~\cite{kaufman2009finding}.
...
The y-axis presents the average Jaccard dissimilarity between each pair of
views.}
...
and $D=5$, varied the level of deduplication and measured the diversity of the
views. To change the level of deduplication, we varied the value of $B'$
between $B$ and $N.B$ (cf. Section \ref{sec:variety}). A level of 0\% means
that $B'=B$. A level of 100\% means that $B'=N.B$. To quantify the diversity of
the views, we measured the pariwise Jaccard dissimilarity between every pair of
views and averaged the results. We observe that the strategy works in all four
cases, but with different levels of efficiency. In the \texttt{BankMarketing}
case, our strategy almost doubles the pairwise dissimilarity of the views. The
effect is much lighter on datasets with few columns, such as \texttt{USCensus}
and \texttt{MAGICTelescope}.
