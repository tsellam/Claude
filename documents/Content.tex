...
datasets? We must design efficient heuristics and approximations to quickly
traverse all the combinations of all the dimensions.
...
practical methods to detect good views in large data\-bases, based on
approximations and greedy search.  (3) We apply our model to practical
situations, involving either humans (data exploration by subgroup discovery) or
automatic prediction (classification algorithms).
...
which illustrates unusual variations of the measure.
...
and lower employment rates tend to be safer (cf.  $POI_2$). We see that
\texttt{Unemployment} and \texttt{Population Density} are interesting because
they influence the target \texttt{Crime}.  The POIs highlight this influence.

Interesting views and POIs are the key concepts behind our study. We
...
is the first problem we will tackle. Then, given this framework, we
...
We introduced our notions of views and POIs. We now present elementary notions
of information theory~\cite{cover2012elements}, and derive our model of
interestingness. In the remaining of this paper, we suppose that each column
$X_n = (x_n^1, \ldots, x_n^M)$ contains $M$ samples drawn from a random
variable $\rv{X}_n$ with sample space $\Omega_n$.

\subsection{View Strength}
\label{sec:view-strength}
Let us define what makes a view ``interesting''. A set of columns is
...
unemployment leads to higher levels of crime. We can model this relationship
with \emph{mutual information}, which we present below.
...
called \emph{mutual information}.  Formally, if $\rv{X}$ and $\rv{T}$ are two
...
$\rv{T} | \rv{X}$. It is symetic, and it is always positive or null. 

We can now expose the most basic idea behind our study. A variable $\rv{X}$ is
interesting if $I(\rv{X}; \rv{T})$ is as high as possible.  We generalize this
notion to several variables: a set of variables $\rv{X}_1, \ldots, \rv{X}_D$ is
interesting they are \emph{jointly} dependent to the target, i.e., $I(\rv{X}_1,
\ldots, \rv{X}_D ; \rv{T})$ is high.
\begin{definition}
...
we have access to the samples $X_n$. Therefore, we compute \emph{estimates} of
...
property it with the lemma below.
\begin{lemma}
...
Our framework allow us to formalize the problem of generating hypotheses:
...
selection from subgroup search could benefit the a wide range of subgroup
